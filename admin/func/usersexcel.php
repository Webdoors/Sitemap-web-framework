...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N1', 'specprice');
...
$q1=mysqli_query($con,"SELECT t1.*,(SELECT COUNT(t4.id) FROM orders as t4 WHERE t4.uid=t1.id AND t4.status='4') as 'tsum',(SELECT t5.title_ka FROM filters as t5 WHERE t5.id=t1.price_group LIMIT 1) as 'pgroup',(SELECT count(t5.id) FROM specprices as t5 WHERE t5.uid=t1.id) as 'specprice' FROM id as t1 WHERE t1.id>0  $WHERE  
 ORDER BY t1.id DESC");
$i=1;
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('N'.$i, $r1['specprice']);


}
			// Redirect output to a client’s web browser (Excel2007)
			header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
			header('Content-Disposition: attachment;filename="'."UsersReport_".date('U').'.xlsx');
