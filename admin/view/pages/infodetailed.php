...
if($a!=""&&$a!="0"){
	// echo "SELECT * FROM info WHERE id='".$a."' ";
	$q1=mysqli_query($con,"SELECT * FROM info WHERE id='".$a."' ");
...
// echo "INSERT INTO info (date,authorid) VALUES ('$T','$Guid') ";
	$ina=mysqli_query($con,"INSERT INTO info (date,authorid,sitemapid) VALUES ('$T','$Guid','$sid') ");
	$pid=mysqli_insert_id($con);
	mysqli_query($con,"UPDATE sitemap SET name_ka='info_sitemapid-".$sid."' WHERE id='".$sid."'");
	$dd=mysqli_query($con, "SELECT id FROM info WHERE news!=1 ORDER BY id DESC");
...
<script>location.href="?page=infodetailed&id=<?=$a?>"</script>
<?php
...
<h3 class="title mt-3">
 ...
 <button  class="btn float-right mb-2 btn-success SAVESET" style="float:right" msg="შენახულია"   sitemap='2' t='info' pagename='blog' d="<?=$a?>">დამახსოვრება</button>
 <a target="_blank" href="/<?=$L?>/article/<?=isset($r2["slug_".$L])?$r2["slug_".$L]:""?>"class="btn float-right mb-2 btn-primary me-1" style="float:right"   sitemap='2' t='info' pagename='blog' d="<?=$a?>">ნახვა</a>
<div class="row">
...
<div role="tabpanel">
    ...
    <div class="tab-content  itmcontainer" t='info'>
        ...
        <div role="tabpanel" class="tab-pane active" id="georgian">
           ...
           <div class='enebi' d='' >
                       ...
                       <input class="form-control UPTNO" t="info" d="<?=$a?>" n="name_<?=$L?>" value="<?=$r1["name_".$L] ?>" tp=''  rows="10"  id="ADLGE" name="title" cols="50">
		               <br>
               </div>	
           <div class='enebi col-sm-2' d='' >
                       <label for="ge[sarchevi]" class="required">თარიღი <?=$L ?></label>
                       <input class="form-control UPTNO" t="info" d="<?=$a?>" n="date_<?=$L?>" value="<?=$r1["date_".$L] ?>" tp=''  rows="10" placeholder="november 11, 2024"  id="ADLGE" name="title" cols="50">
		               <br>
               </div>		
...
<div class="row my-3">
	...
	<div class="col-sm-3">
			<input class="form-control UPT" d="<?=$r1["id"]?>" onchange="$('.SITEM').attr('href','/admin/?page=sitemapedit&id='+$(this).val())" n="sitemapid" t="info" value="<?=$r1["sitemapid"]?>"/>
	</div>
	<div class="col-sm-4">
...
<div class="row my-3">
	<div class="col-sm-2">
		<small>ინფოს სურათი <?=$L?></label> <small></small>
	</div>
	<div class="col-sm-7">

<div class="input-group">
<input type="text" class="form-control UPTNO IMAGE" d="<?=$r1["id"]?>"  n="img_<?=$L?>" t="info"  value="<?=$r1["img_".$L]?>" id="example1<?=$a ?>" aria-describedby="minipan">
...
<div class="input-group">
<input type="text" class="form-control UPTNO IMAGE" d="<?=$r1["id"]?>"  n="video_<?=$L?>" t="info"  value="<?=$r1["img_".$L]?>" id="example2<?=$a ?>">
<div class="input-group-append">
  ...
  <br/>
        ...
        <div class='enebi' d='<?=$c ?>' >
           <div class="col-sm-12 pad-no-right pad-no-xs NOP">
               ...
               <textarea id="ADTENG" tiny='1' t="info" d="<?=$a?>" n="text_<?=$L?>" class=' tnmc ADTGE' tp='' name='text' placeholder="ტექსტი"><?=$r1["text_".$L] ?></textarea>
        ...
        <div class='enebi mt-3' d='<?=$c ?>' >
           <div class="col-sm-12 pad-no-right pad-no-xs NOP">
               ...
               <textarea   t="info" d="<?=$a?>" n="description_<?=$L?>" class='form-control UPTNO' tp='' name='text' placeholder="მცირე ტექსტი"><?=$r1["description_".$L] ?></textarea>
  
            </div>     
		</div>	
<div class="row my-3">
	<div class="col-sm-2">
		<small>ინფოს სურათების დამატება <?=$L?></label> <small></small>
	</div>
	<div class="col-sm-7">

<div class="input-group">
<input type="text" class="form-control  POSTIMG" d="<?=$r1["id"]?>"  n="img" t="info"  value="" id="example3<?=$a ?>" aria-describedby="minipan">
<div class="input-group-append">
...
<?php
$q22=mysqli_query($con,"SELECT * FROm infoimgs WHERE postid='".$r1["id"]."' ORDER BY id DESC ");
while($r22=mysqli_fetch_array($q22)){
?>
	<div class="col-sm-12 mt-2">
<a href="<?=$r22["img"]?>" class="d-inline-block " target="_blank" ><img style="width:120px" src="<?=$r22["img"]?>"/></a> <div class="ms-2 btn btn-danger DEL" d="<?=$r22["id"]?>" n="infoimgs"><i class="fa fa-trash text-white"></i></div>
	</div>
<?php
}
?>	
...
<div class="row my-3">
	...
	<div class="col-sm-7">
		...
		<label>
		    აქტიური
		   <input type='checkbox' class='UPTNO' n='active' d="<?=isset($r2["id"])?$r2["id"]:""?>" t="info" tp='int' ln='' <?=$r1['active']=='1'?'checked':'' ?> class='aactv' />
		</label>   
    ...
    <div class="submit my-4">
        <button  class="btn btn-success SAVESET" msg="შენახულია"   sitemap='2' t='info' pagename='blog' d="<?=$a?>">დამახსოვრება</button>
    </div>
	  ...
	  style_formats: [{
		...
		setup : function(ed) {
			  ed.on('change keyup', function(e) {
				  ...
				  func("updatetable","info",$("#"+ed.id).attr("n"),ed.getContent(),$("#"+ed.id).attr("d"));
