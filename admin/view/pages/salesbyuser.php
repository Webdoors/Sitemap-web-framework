...
$q1=mysqli_query($con,"SELECT t1.*,(SELECT COUNT(t4.id) FROM orders as t4 WHERE t4.uid=t1.id AND t4.status='4') as 'tsum',(SELECT SUM(t4.amount) FROM orders as t4 WHERE t4.uid=t1.id AND t4.status='4') as 'totalsales',(SELECT t3.name FROM usertypes as t3 WHERE t1.usertype=t3.id) as 'utype' FROM id as t1 WHERE t1.id>0 $WHERE  ORDER BY t1.id DESC LIMIT ".$PA." OFFSET ".$fr.""); 

// echo "SELECT t1.*,(SELECT COUNT(t4.id) FROM orders as t4 WHERE t4.uid=t1.id AND t4.status='4') as 'tsum',(SELECT SUM(t4.amount) FROM orders as t4 WHERE t4.uid=t1.id AND t4.status='4') as 'totalsales',(SELECT t3.name FROM usertypes as t3 WHERE t1.usertype=t3.id) as 'utype' FROM id as t1 WHERE t1.id>0 $WHERE  ORDER BY t1.id DESC LIMIT ".$PA." OFFSET ".$fr."";
...
$q101=mysqli_query($con,"SELECT SUM((SELECT SUM(t4.amount) FROM orders as t4 WHERE t4.uid=t1.id AND t4.status='4')) as 'toto' FROM id as t1 WHERE t1.id>0 $WHERE  "); 
$r101=mysqli_fetch_array($q101);
$q3=$q100;
...
<div class="row justify-content-center">
	<div class="col-sm-12">
		<div class="row my-2">
			...
			<div class="col-sm-2">
				<form><input type="hidden" name="page" value="salesbyuser"/><input class="form-control SERKEY2" name="key" placeholder="Search" value="<?=$KEY?>"/></form>
			</div>
			<div class="col-sm-1">
			...
			<div class="col-sm-3">
<input type="text" class="form-control INP" n="deadline" d="?page=salesbyuser&type=<?=$stype??''?>&sort=<?=$srt??''?>&status=<?=$status?>" name="daterange" value="<?=$start!=""?substr($start,6,2)."/".substr($start,4,2)."/".substr($start,0,4)." - ".substr($end,6,2)."/".substr($end,4,2)."/".substr($end,0,4):""?>" placeholder="" />
			</div>
			<div class="col-sm-2 ">
				<select class="form-select UTY" onchange="location.href='?page=salesbyuser&spero=<?=$spero?>&key=<?=$key?>&start=<?=$start?>&end=<?=$end?>&status='+$(this).val()+'&active='+$('.UAC').val()">
			...
			<div class="col-sm-2 px-0">
				<select class="form-select UAC" onchange="location.href='?page=salesbyuser&spero=<?=$spero?>&key=<?=$key?>&start=<?=$start?>&end=<?=$end?>&status='+$('.UTY').val()+'&active='+$(this).val();">
			...
			<div class="col-sm-1">
				<a href="?page=salesbyuser" class="btn btn-danger"><i class="fa text-white fa-sync"></i></a>
			</div>
			<div class="col-sm-1 d-flex justify-content-end">
<a tagret="_blank" href="/admin/func/salesbyuserexcel.php?cid=<?=$cid!=""?$cid:""?>&spero=<?=$spero?>&status=<?=$status?>&start=<?=$start?>&end=<?=$end?>&active=<?=$active?>"><button class="btn btn-success">Excel</button>	</a>			
			</div>
		</div>
	</div>
	<div class="col-sm-12">
...
?>
		  <tr>
			...
			<th class="">
				<h5><span class='badge bg-warning'><?=number_format(($r1["totalsales"]??0),2)?></span></h5>
			</th>			
		  </tr>
<?php
$cou=$cou-1;
...
?>
	...
	<div class='row'>
	    <div class='col-md-9'>
	<a href="?page=salesbyuser&p=1&cid=<?=$cid!=""?$cid:""?>&key=<?=$key?>&spero=<?=$spero?>&status=<?=$status?>&start=<?=$start?>&end=<?=$end?>&active=<?=$active?>" class="PG USR">«</a>
	<a href="?page=salesbyuser&p=<?=$ACP!=1?($ACP-1):$ACP?>&key=<?=$key?>&spero=<?=$spero?>&cid=<?=$cid!=""?$cid:""?>&status=<?=$status?>&start=<?=$start?>&end=<?=$end?>&active=<?=$active?>" class="PG USR">‹</a>
	<?php
	for($i=1;$i<=ceil(mysqli_num_rows($q3)/$PA);$i++){
		if(($ACP-5)<=$i&&($ACP+5)>=$i){
	?>
	<a href="?page=salesbyuser&p=<?=$i?>&cid=<?=$cid!=""?$cid:""?>&key=<?=$key?>&spero=<?=$spero?>&status=<?=$status?>&start=<?=$start?>&end=<?=$end?>&active=<?=$active?>" class="PG <?=($ACP==$i?"ACP":"")?> USR"><?=$i?></a>
	<?php }
	}
	?>
	<a href="?page=salesbyuser&p=<?=$ACP!=ceil(mysqli_num_rows($q3)/$PA)?($ACP+1):$ACP?>&key=<?=$key?>&spero=<?=$spero?>&cid=<?=$cid!=""?$cid:""?>&status=<?=$status?>&start=<?=$start?>&end=<?=$end?>&key=<?=$key?>&active=<?=$active?>" class="PG USR">›</a>
	<a href="?page=salesbyuser&p=<?=ceil(mysqli_num_rows($q3)/$PA);?>&key=<?=$key?>&cid=<?=$cid!=""?$cid:""?>&spero=<?=$spero?>&status=<?=$status?>&start=<?=$start?>&end=<?=$end?>&active=<?=$active?>" class="PG USR">» <?=ceil(mysqli_num_rows($q3)/$PA);?></a>
	</div>
	<div class='col-md-1'>
	   <h5>სულ <?=$Total ?></h5>
	</div>
	<div class='col-md-1'>
	   <h5> <?=number_format($r101["toto"],2) ?>₾</h5>
	</div>
	</div>
	</div>
</div>
...
$(function() {
	$(".SELSPERO").on("change",function(){
		location.href="?page=salesbyuser&spero="+$(this).val()+"&start=<?=$start?>&end=<?=$end?>&status="+$(".UTY").val()+"&active="+$(".UAC").val();
	});
  $('input[name="daterange"]').daterangepicker({
	...
	location.href=$('input[name="daterange"]').attr("d")+"&spero="+$(".SELSPERO").val()+"&start="+start.format('YYYYMMDD')+"&end="+end.format('YYYYMMDD')+"&status="+$(".UTY").val()+"&active="+$(".UAC").val();
